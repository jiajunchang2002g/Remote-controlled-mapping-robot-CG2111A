	...
	switch(packet->command)
	{
		...
		case RESP_COLOUR:
			switch(packet->params[0])
			{
				...
				default:
					printf("R: %d, G: %d, B: %d", packet->params[1], packet->params[2], packet->params[3]);
			}
			break;
...
void getParams(TPacket *commandPacket, char command)
{
...
int main()
{
	...
	{
		char ch;
		printf("Command (w=forward, s=reverse, a=turn left, d=turn right, e=stop, c=clear stats, g=get stats, q=exit, r = rgb)\n");
		scanf("%c", &ch);
